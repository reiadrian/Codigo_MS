...
%             NPG_ele = 1;
      ...
      switch e_DatMatSet.conshyp
         ...
         case {4,5,10,11,12}
            ...
            fprintf(fid,['# Tabla de resultados del Set_',num2str(iSet),'\n']);
            %Tabla para los Factores de carga (para verlo en el GID hay que usar Contour Ranges.
            fprintf(fid_res,['ResultRangesTable ','"Factor de Carga de Daño Isotropico"\n']);
            fprintf(fid_res,' - -0.5: "-1: Elastica (con daño)"\n');
            fprintf(fid_res,'-0.5 - 0.5: "0: Elastica (sin daño)"\n');
            fprintf(fid_res,'0.5 - 1.5: "1: Daño"\n');
            fprintf(fid_res,'1.5 - : "2: Sobre limite de daño"\n');
            fprintf(fid_res,'End ResultRangesTable\n');
         case {50,51,53,54,55}
