function    [e_TanOp,m_VarFluc_eps0,m_VarFluc_p0,m_VarFluc_phi0] ...
    = f_OpTangHomBifase3(KT,c_CT,m_LinCond,dofl,doff,e_DatSet,Omega_micro,OmegaMicro_pm,...
...
%    if e_VG.conshyp==14||e_VG.conshyp==15||e_VG.conshyp==16
...
%    end
   ...
   m_Csig_eps_sm = zeros(ntens,ntens);
   m_Csig_eps_pm = zeros(ntens,ntens);
   ...
   m_bsig_p_sm = zeros(ntens,1);
   m_bsig_p_pm = zeros(ntens,1);
   ...
   m_ksig_phi_sm = zeros(ntens,2);
   m_ksig_phi_pm = zeros(ntens,2);
        ...
        for iElem = 1:nElem
            ...
            if m_ElemLocSet(iElem)
                ...
                for iPG = 1:npg
                    % %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    % Operadores tangentes respecto de las deformaciones (eps)
                    m_Csig_eps_pm = m_Csig_eps_pm+(m_CT(:,:,iPG,iElem)*(m_IndTens+...
                    ...
                    % Operadores tangentes respecto de las poropresiones (p)
                    m_bsig_p_pm = m_bsig_p_pm+(m_CT(:,:,iPG,iElem)*m_BT(:,:,iPG,iElem)*m_VarFlucElem_p_u-...
                    ...
                    % Operadores tangentes respecto del gradiente de las poropresiones (phi)
                    m_ksig_phi_pm = m_ksig_phi_pm+(m_CT(:,:,iPG,iElem)*m_BT(:,:,iPG,iElem)*m_VarFlucElem_phi_u-...
    ...
    elseif e_DatSet(iSet).e_DatMat.conshyp==1
      ...
      for iElem = 1:nElem
         ...
         if m_ElemLocSet(iElem)
            ...
            dofElem = m_DofElem(:,iElem);
            m_pesoPG = m_DetJT(:,iElem).*wg;
            m_VarFlucElem_eps_u = m_VarFluc_eps(dofElem,:);
            m_VarFlucElem_p_u = m_VarFluc_p(dofElem,:);
            m_VarFlucElem_phi_u= m_VarFluc_phi(dofElem,:);
% % %             m_VarFlucElem = m_VarFluc(dofElem,:);
            for iPG = 1:npg
% %                 m_CTHomog = m_CTHomog+m_CT(:,:,iPG,iElem)*(m_IndTens+...
% %                   m_BT(:,:,iPG,iElem)*m_VarFlucElem)*m_pesoPG(iPG);
               m_Csig_eps_sm = m_Csig_eps_sm+m_CT(:,:,iPG,iElem)*(m_IndTens+...
                  m_BT(:,:,iPG,iElem)*m_VarFlucElem_eps_u)*m_pesoPG(iPG);
              
              m_bsig_p_sm = m_bsig_p_sm+(m_CT(:,:,iPG,iElem)*m_BT(:,:,iPG,iElem)*m_VarFlucElem_p_u)*m_pesoPG(iPG);
              
              
              m_ksig_phi_sm = m_ksig_phi_sm+(m_CT(:,:,iPG,iElem)*m_BT(:,:,iPG,iElem)*m_VarFlucElem_phi_u)*m_pesoPG(iPG);
            end
         end
      end
    end
   ...
   m_Csig_eps = (m_Csig_eps_sm+m_Csig_eps_pm)/(Omega_micro);
   m_bw_eps = m_bw_eps/OmegaMicro_pm;
   m_Cw_eps3 = m_Cw_eps3*theta*Dtime/OmegaMicro_pm;
   m_Cw_eps2 = m_Cw_eps2/OmegaMicro_pm;
   m_Cw_eps3 = m_Cw_eps3+m_Cw_eps2;
   m_Cw_eps = m_Cw_eps/OmegaMicro_pm;
    
%    m_bsig_p = (m_bsig_p_sm+m_bsig_p_pm)/(Omega_micro);
   m_bsig_p = (m_bsig_p_pm)/(OmegaMicro_pm);
   
   m_Mww_p = m_Mww_p/OmegaMicro_pm;
   m_bw_p3 = m_bw_p3*theta*Dtime/OmegaMicro_pm;
   m_bw_p2 = m_bw_p2/OmegaMicro_pm;
   m_bw_p3 = m_bw_p3+m_bw_p2;
   
   m_bw_p = m_bw_p/OmegaMicro_pm;
      
%    m_ksig_phi = (m_ksig_phi_sm+m_ksig_phi_pm)/(Omega_micro);
      m_ksig_phi = (m_ksig_phi_pm)/(OmegaMicro_pm);
   
   m_kww_phi = m_kww_phi/OmegaMicro_pm;
   m_kw_phi3 = m_kw_phi3*theta*Dtime/OmegaMicro_pm;
   m_kw_phi2 = m_kw_phi2/OmegaMicro_pm;
   m_kw_phi3 = m_kw_phi3+m_kw_phi2;
  
   m_kw_phi = m_kw_phi/OmegaMicro_pm;
...
% % %           pos_d =  e_DatElemSet.pos_d;
          ...
          for iElem = 1:nElem        
             ...
             if m_ElemLocSet(iElem)
                ...
                m_FCHomElem_Ct = zeros(dofpe,ntens);
% % %                 m_FCHomElem_Biot = zeros(dofpe_p,ntens);
                for iPG = 1:npg
                   %No se coloca el signo menos porque en incremental_disp cuando se resuelve el sistema ya
                   %se lo pone.
                   m_FCHomElem_Ct = m_FCHomElem_Ct+m_BT(:,:,iPG,iElem)'*m_CT(:,:,iPG,iElem)*m_pesoPG(iPG);
                end
                %dofElem = f_DofElem(conec(iElem,:),ndn);
                dofElem = m_DofElem(:,iElem);
                m_FCHom(dofElem,:) = m_FCHom(dofElem,:)+m_FCHomElem_Ct;
                %%%%m_FCHom(dofElem(pos_d),:) = m_FCHom(dofElem(pos_d),:)+m_FCHomElem_Ct;
             end
          end
      end
...
% % %                         m_pesoPG = m_DetJT(:,iElem).*wg;
% % % 
% % %                         m_FCHomElem_Bi = zeros(dofpe,1);
% % %         % % %                 m_FCHomElem_Biot = zeros(dofpe_p,ntens);
% % %                         for iPG = 1:npg
% % %                            %No se coloca el signo menos porque en incremental_disp cuando se resuelve el sistema ya
% % %                            %se lo pone.
% % %                            m_FCHomElem_Bi = m_FCHomElem_Bi+m_BT(:,:,iPG,iElem)'*BiotM*m_pesoPG(iPG);
% % %                         end
% % %                         %dofElem = f_DofElem(conec(iElem,:),ndn);
% % %                         dofElem = m_DofElem(:,iElem);
% % %                         m_FCHom(dofElem,:) = m_FCHom(dofElem,:)+m_FCHomElem_Bi;
% % %                         %%%%m_FCHom(dofElem(pos_d),:) = m_FCHom(dofElem(pos_d),:)+m_FCHomElem_Ct;
% % %                      end
% % %                  end
       end
...
% % %                            %se lo pone.
% % %                            m_FCHomElem_Biy = m_FCHomElem_Biy-(m_BT(:,:,iPG,iElem)'*BiotM*...
% % %                            N4(:,:,iPG)*(yy(1:2,1:4))')*m_pesoPG(iPG);
% % %                         end
% % %                         %dofElem = f_DofElem(conec(iElem,:),ndn);
% % %                         dofElem = m_DofElem(:,iElem);
% % %                         m_FCHom(dofElem,:) = m_FCHom(dofElem,:)+m_FCHomElem_Biy;
% % %                         %%%%m_FCHom(dofElem(pos_d),:) = m_FCHom(dofElem(pos_d),:)+m_FCHomElem_Ct;
% % %                      end
% % %                  end
       end
